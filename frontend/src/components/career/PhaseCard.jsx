...
// Updated: shows ML-annotated "hasSkill" flag (user already knows this skill,
// detected by semantic similarity in the backend ML pipeline).
//
// SKILL_LEVELS:  index → label (matches backend build_phases order)
//   [0] Beginner  [1] Elementary  [2] Intermediate
...
const SKILL_LEVELS = [
  { label: 'Beginner',     sublabel: 'Start here — foundations & setup',      icon: '🌱', color: G.green,   bg: G.greenBg,       border: G.greenBd },
  { label: 'Elementary',   sublabel: 'Build working knowledge',               icon: '📖', color: '#0891b2', bg: '#ecfeff',       border: '#a5f3fc' },
  { label: 'Intermediate', sublabel: 'Real-world tools & patterns',           icon: '⚙️', color: G.blue,    bg: G.blueBg,        border: G.blueBd  },
  { label: 'Advanced',     sublabel: 'Production-grade expertise',            icon: '🔥', color: G.amber,   bg: G.amber + '12',  border: G.amber + '35' },
  { label: 'Expert',       sublabel: 'Capstone projects & portfolio',         icon: '🏗️', color: '#7c3aed', bg: '#f5f3ff',       border: '#ddd6fe' },
  { label: 'Master',       sublabel: 'Interview ready — land the job',        icon: '🎯', color: '#dc2626', bg: '#fef2f2',       border: '#fecaca' },
];
...
function ResourceChip({ resource }) {
  return (
    <a
      ...
      style={{
        fontSize: 11, color: G.blue, textDecoration: 'none',
        padding: '4px 6px', borderRadius: 3, background: G.blueBg,
...
function TaskRow({ item, phaseKey, resourceData, checked, saving, onToggle }) {
  ...
  return (
    <div style={{
      borderRadius: 5,
      border: `1px solid ${hasSkill ? G.greenBd : done ? G.greenBd : G.border}`,
      background: hasSkill ? '#f0fdf4' : done ? G.greenBg : G.bg,
      overflow: 'hidden',
      position: 'relative',
    }}>
      {/* ML "you have this" ribbon */}
      {hasSkill && !done && (
        <div style={{
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: 1 }}>
          ...
          <div style={{
            ...
            border: `1.5px solid ${done ? G.green : hasSkill ? G.green + '80' : G.border2}`,
            background: done ? G.green : hasSkill ? G.green + '20' : G.white,
          ...
          <div style={{ flex: 1 }}>
            <span style={{
              ...
              color: done ? G.green : hasSkill ? '#166534' : G.text,
...
export default function PhaseCard({ phase, phaseIndex, checked, saving, onToggle }) {
  ...
  // Count ML-detected already-known skills in this phase
  const alreadyKnownCount = resourcesList.filter((r) => r?.hasSkill).length;
        ...
        <div style={{
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 5 }}>
              ...
              {/* ML annotation: already known skills */}
              {alreadyKnownCount > 0 && (
                <span style={{
                  fontSize: 10, padding: '2px 8px', borderRadius: 20,
                  background: G.greenBg, color: G.green,
                  border: `1px solid ${G.greenBd}`, fontWeight: 600,
                }}>
                  🤖 {alreadyKnownCount} skill{alreadyKnownCount > 1 ? 's' : ''} you already know
                </span>
              )}
            </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 6 }}>
            <span className="badge" style={{
              ...
              border: `1px solid ${phaseColor}30`, fontWeight: 700, fontSize: 11,
            }}>
              {phaseDone}/{tasks.length}
            </span>
            <div style={{ width: 60 }}>
              <div style={{ height: 3, borderRadius: 999, background: level.border, overflow: 'hidden' }}>
                <div style={{
                  height: '100%', borderRadius: 999,
                  width: `${tasks.length ? (phaseDone / tasks.length) * 100 : 0}%`,
                  background: phaseColor, transition: 'width 0.4s ease',
                }} />
              </div>
            </div>
          </div>
