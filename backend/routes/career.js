...
async function generateDynamicRoadmap(role, userId, location = 'India', refresh = false) {
  try {
    ...
    if (!refresh) {
      ...
      // On explicit refresh — wipe in-memory cache
      delete global[cacheKey];
      console.log(`[DynamicRoadmap] Cache WIPED for refresh: ${role}`);
    }
...
router.delete('/cache', auth, async (req, res) => {
  try {
    ...
    const cacheKey = `roadmap_v5_${role.toLowerCase().replace(/[^a-z0-9]/g, '_')}_india`;
    const existed = !!global[cacheKey];
    delete global[cacheKey];
    res.json({ deleted: existed, role, message: 'In-memory cache cleared. Next fetch will regenerate.' });
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
});
